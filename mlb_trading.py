...
for index, row in combined_table.iterrows():
  ...
  incr_tag = f'{increment1}c' if is_tail else f'{increment}c'
  print(f"\n{'='*70}")
  ...
  # --- Strategy params ---
  print(f"    Hi NO price={hi_no} | Position={position} | {mode_tag} mode (incr={incr_tag}, {starting_contracts}c/level)")
  ...
  total_contracts = sum(c for _, c, _ in order_levels)
  print(f"    -> NO: [{display_str}] ({total_contracts} contracts across {len(order_levels)} levels)")
...
print(f"  Contracts per level:    {starting_contracts}")
print(f"  Increment (between):    {increment}c")
print(f"  Increment (tail):       {increment1}c")
