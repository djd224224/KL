...
def check_scan_perf_staleness(alerter, state: dict, now: str) -> None:
    ...
    try:
        ...
        alerter.send_message(
            ...
            f"treats every event root, series and family as unknown, so the "
            f"open scan tier admits and ranks on the model's ROI alone (no "
            f"block, no blend). Nothing breaks "
