...
All parameters adapted from NCAAB P&L analysis. Core functions carried from NBA v4.2.
"""
...
# =========================
MAX_NET_PER_MARKET = 150         # [NCAAB-3] Was 300 — halved to limit blowups
POSITION_MODERATE_THRESHOLD = 75  # [NCAAB-6] Was 100
POSITION_STOP_THRESHOLD = 150     # [NCAAB-6] Was 300
MAX_ORDERBOOK_LEVELS_ABOVE = 2

MAX_NET_PER_EVENT = 800           # [NCAAB-3] Was 2000 — caps game exposure ~$150
...
# =========================
# [NCAAB-2] SIDE_MULTIPLIERS — completely rewritten from analysis
# =========================
SIDE_MULTIPLIERS = {
    # --- BLOCKED (analysis showed consistent losses) ---
    "DOUB": {"yes": 0.0, "no": 0.0},   # [NCAAB-2] Biggest loser: -$191, -28% ROI, coin-flip variance
    "TRAN": {"yes": 0.0, "no": 0.0},   # [NCAAB-2] 79% yes rate → No structurally loses. -$64 P&L
    "WALK": {"yes": 0.0, "no": 0.0},   # [NCAAB-2] -60% ROI, tiny sample, unreliable
    "AIRB": {"yes": 0.0, "no": 0.3},   # [NCAAB-2] -22.5% ROI. Mostly skip, small No only

    # --- SAFE BUNDLE: strong No markets (proven profitable) ---
    "ALLE": {"yes": 0.0, "no": 2.0},   # [NCAAB-2] Best: +$144, +30% ROI, 21% base yes
    "MARC": {"yes": 0.0, "no": 1.8},   # [NCAAB-2] +$38, +6.5% ROI, 13% base yes
    "NIL":  {"yes": 0.0, "no": 1.8},   # [NCAAB-2] +$35, +15.4% ROI, 18% base yes
    "RECO": {"yes": 0.0, "no": 1.5},   # [NCAAB-2] +$74, +15.5% ROI, 56.5% base yes
    "ALL":  {"yes": 0.0, "no": 1.5},   # [NCAAB-2] +$42, +13.2% ROI, 53.3% base yes
    "ELBO": {"yes": 0.0, "no": 1.3},   # [NCAAB-2] +$25, +15.6% ROI, 64.5% base yes

    # --- SECONDARY: mild No, underperforming but not losers ---
    "DRAF": {"yes": 0.0, "no": 0.8},   # [NCAAB-2] -$26, -5.4% ROI — demoted
    "RECR": {"yes": 0.0, "no": 0.7},   # [NCAAB-2] -$9, -1.8% ROI — underperforming
    "ANKL": {"yes": 0.0, "no": 0.7},   # [NCAAB-2] +$2, +0.6% ROI — barely positive
    "SCHE": {"yes": 0.0, "no": 0.7},   # [NCAAB-2] -$7, -4.5% ROI — underperforming
    "OVER": {"yes": 0.0, "no": 1.0},   # [NCAAB-2] +$3, +0.9% ROI — neutral, No-only
}
...
TEAM_MARKET_OVERRIDES = {
    # ALLE (base 21% yes) — specific teams dramatically change odds
    ("ALA", "ALLE"): {"yes_mult": 1.5, "no_mult": 0.0},   # ALA triggers 100% → flip to Yes
    ("FLA", "ALLE"): {"yes_mult": 1.2, "no_mult": 0.0},   # FLA triggers 75% → flip to Yes

    # RECO (base 56.5% yes) — MIC never triggers, some teams always trigger
    ("MIC", "RECO"): {"yes_mult": 0.0, "no_mult": 2.5},   # MIC = 0% → strong No
    ("BYU", "RECO"): {"yes_mult": 0.0, "no_mult": 0.0},   # BYU = 100% → skip (No will lose)
    ("ARK", "RECO"): {"yes_mult": 0.0, "no_mult": 0.0},   # ARK = 100% → skip
    ("ILL", "RECO"): {"yes_mult": 0.0, "no_mult": 0.0},   # ILL = 100% → skip

    # MARC (base 12.9% yes) — IND much higher
    ("IND", "MARC"): {"yes_mult": 0.0, "no_mult": 0.5},   # IND = 50% → weaken No

    # DRAF (base 55.7% yes) — several teams at extremes
    ("VAN", "DRAF"): {"yes_mult": 0.0, "no_mult": 2.0},   # VAN = 0% → strong No
    ("ILL", "DRAF"): {"yes_mult": 0.0, "no_mult": 2.0},   # ILL = 0% → strong No
    ("NEB", "DRAF"): {"yes_mult": 0.0, "no_mult": 2.0},   # NEB = 0% → strong No
    ("PUR", "DRAF"): {"yes_mult": 0.0, "no_mult": 1.5},   # PUR = 20% → boost No
    ("ALA", "DRAF"): {"yes_mult": 0.0, "no_mult": 0.0},   # ALA = 100% → skip
    ("DUK", "DRAF"): {"yes_mult": 0.0, "no_mult": 0.0},   # DUK = 100% → skip
    ("FLA", "DRAF"): {"yes_mult": 0.0, "no_mult": 0.0},   # FLA = 100% → skip

    # DOUB (base 49.2%) — blocked globally but if re-enabled:
    ("FLA", "DOUB"): {"yes_mult": 0.0, "no_mult": 0.0},   # FLA = 100% → never No
    ("MSU", "DOUB"): {"yes_mult": 0.0, "no_mult": 0.0},   # MSU = 100% → never No
    ("VAN", "DOUB"): {"yes_mult": 0.0, "no_mult": 1.5},   # VAN = 0% → safe No
    ("USC", "DOUB"): {"yes_mult": 0.0, "no_mult": 1.5},   # USC = 0% → safe No

    # TRAN (base 79%) — blocked globally but if re-enabled:
    ("ALA", "TRAN"): {"yes_mult": 0.0, "no_mult": 1.5},   # ALA = 33% → rare profitable No
    ("NEB", "TRAN"): {"yes_mult": 0.0, "no_mult": 1.2},   # NEB = 40% → mild No
}
...
MIN_PRICE_NO = 12                 # [NCAAB-6] Was 5 — eliminates sub-$10 positions (-11.7% ROI)
...
# =========================
NO_SWEET_SPOT_MIN = 50            # [NCAAB-4] Was 35 — 50-65¢ = 61% WR, 65-80¢ = 86% WR
NO_SWEET_SPOT_MAX = 80            # [NCAAB-4] Was 65
NO_SWEET_SPOT_MULTIPLIER = 1.8    # [NCAAB-4] Was 1.5 — boosted given strong edge
...
def generate_base_contracts(num_levels: int) -> List[int]:
    """[NCAAB-9] Reduced from 15/20/25/30 to 10/12/15/18 to prevent oversized positions."""
    contracts = []
    for i in range(num_levels):
        if i < 3:
            contracts.append(10)     # Was 15
        elif i < 5:
            contracts.append(12)     # Was 20
        elif i < 8:
            contracts.append(15)     # Was 25
        else:
            contracts.append(18)     # Was 30
    return contracts
...
MAX_CONTRACTS_PER_ORDER = 500     # Was 1000
