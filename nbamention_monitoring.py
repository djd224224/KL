#!/usr/bin/env python3
# =============================================================================
# KXNBAMENTION MONITORING SCRIPT -- GitHub Actions Compatible
# =============================================================================
...
def load_orders_from_bigquery(series_ticker: str) -> pd.DataFrame:
    ...
    # Deduplicate by order_id (WRITE_APPEND creates dupes across runs)
    query = f"""
        SELECT * FROM (
            SELECT
                order_id,
...
def build_settlements_dataframe(settlements, df_fills):
    ...
    #   NO fill  + NO wins:   +(fill_price)           profit = payout - (100 - fill_price) = fill_price
    #   NO fill  + YES wins:  -(100 - fill_price)    loss = actual NO price paid
