#!/usr/bin/env python3
# =============================================================================
# KXMLBMENTION MONITORING SCRIPT -- GitHub Actions Compatible
# =============================================================================
# Pulls fills + settlements from Kalshi API, computes P&L, uploads to BigQuery.
# Run daily to keep reporting tables fresh.
#
# Adapted from KXNBAMENTION monitor with MLB-specific ticker parsing:
#   - HHMM time component in event codes (e.g. 26MAR252005NYYSF)
#   - Variable-length team codes (2 or 3 chars: SF, KC, NYY, BOS)
#
...
def parse_event_code(event_code: str) -> Dict[str, str]:
    ...
    # Detect HHMM time component
    ticker_time_et = ""
    if len(rest) >= 4 and rest[:4].isdigit():
        ticker_time_et = rest[:4]
        teams_code = rest[4:]
    else:
        teams_code = rest
...
def build_fills_dataframe(fills: List[Dict[str, Any]], df_orders: pd.DataFrame) -> pd.DataFrame:
    ...
    # Normalize Kalshi API field names
...
def run_monitoring(series_ticker, upload_to_bq=True):
    ...
    # Step 4: Build df_fills with lineage
    ...
    if len(df_settlements) > 0:
        ...
        print(f"\nSettlements summary:")
        print(f"  Total settled markets: {len(df_settlements)}")
        ...
        # Per-code summary
        if 'ticker_part_3_market_code' in df_settlements.columns:
            filled = df_settlements[df_settlements['num_fills'] > 0]
            if len(filled) > 0:
                print(f"\n  Per-code P&L (settled with fills):")
                code_pnl = filled.groupby('ticker_part_3_market_code').agg(
                    pnl=('pnl', 'sum'),
                    cost=('total_cost', 'sum'),
                    n=('pnl', 'count'),
                ...
                for code, row in code_pnl.iterrows():
                    roi_code = row['pnl'] / row['cost'] * 100 if row['cost'] > 0 else 0
                    print(f"    {code:<8}: P&L=${row['pnl']:>7.2f}, Cost=${row['cost']:>7.2f}, "
                          f"ROI={roi_code:>6.1f}%, {row['wins']}/{row['n']} wins")
