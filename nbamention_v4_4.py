...
def get_scaled_limits(bankroll: float) -> dict:
    ...
    Max scale: 2.0x (at $10,000+). Min scale: 0.5x (at $2,500).
    """
...
def fetch_fills_from_api(
    ...
    # Map yes_price — prefer yes_price_dollars (decimal), fall back to yes_price
    ...
    for _, row in sample.iterrows():
        print(f"    Sample fill: ticker={row.get('ticker')}, side={row.get('side')}, "
              f"yes_price={row.get('yes_price')}, contracts={row.get('contracts', 'N/A')}")
