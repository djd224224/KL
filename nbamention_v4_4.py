...
# Replaces broken Kalshi close_time (now 14 days after game for early-close markets)
# /events endpoint is FREE (zero quota cost)
# =========================
ODDS_API_KEY = os.environ.get("ODDS_API_KEY", "e2d4375f44ff9cc272da93bee587a46a")
ODDS_API_NCAAB_SPORT_KEY = "basketball_ncaab"
ODDS_API_EXPIRATION_BUFFER_SEC = 0  # Orders expire at game start (exact tip-off time)

# Team name → Kalshi 3-letter code mapping
# Order matters: more specific entries checked first (e.g., "michigan state" before "michigan")
NCAAB_TEAM_NAME_TO_CODE = [
    # State-qualified names first (longer match wins)
    ...
    ("arkansas", "ARK"),  # Must precede "kansas" — "arkansas" contains "kansas" as substring
    ...
    ("kentucky", "UKF"),  # Kalshi uses UKF, not KEN
...
# Global caches — populated by fetch_ncaab_schedule()
ncaab_schedule_cache = {}    # "YYYYMMDD-CODE1-CODE2" (sorted) → unix_ts
ncaab_schedule_events = {}   # "YYYYMMDD" → [{"home": str, "away": str, "ts": int}, ...]
...
def ncaab_team_to_code(full_name):
    ...
    for key, code in NCAAB_TEAM_NAME_TO_CODE:
        if key in name_lower:
            return code
    # Fallback: first 3 letters of first word (school name)
...
# Known Kalshi ticker codes that don't prefix-match any word in Odds API team names.
# Maps CODE → substring that WILL appear in the Odds API full team name.
KALSHI_CODE_ALIASES = {
    "SJU":  "john",            # St. John's
    "ILST": "illinois",        # Illinois State (ILST prefix doesn't match any word)
    "UKF":  "kentucky",
    "NCS":  "nc state",
    "MSU":  "michigan state",
    "ISU":  "iowa state",
    ...
    "BOI":  "boise state",
    "MRQ":  "marquette",
    "STJ":  "john",            # alt code for St. John's
    "SMC":  "mary",            # St. Mary's
    "WSU":  "washington state",
    "SCA":  "south carolina",
    "CON":  "connecticut",
    "NOT":  "notre dame",
    # University-abbreviation codes (initials across multiple words)
    "UNM":  "new mexico",      # University of New Mexico
    "NMSU": "new mexico state",# New Mexico State
    "SDSU": "san diego state", # San Diego State (alt code for SDS)
    "ETSU": "east tennessee",  # East Tennessee State
    "MTSU": "middle tennessee",# Middle Tennessee
    "JOES": "joseph",          # Saint Joseph's (explicit alias)
}


def _code_matches_team(code, full_name):
    """Check if a Kalshi ticker code segment plausibly matches an Odds API team name.
    Three strategies: prefix of any word, alias lookup, substring in compressed name."""
    code_upper = code.upper()
    # Clean the name: remove punctuation, split into words
    name_clean = full_name.upper().replace("'", "").replace(".", " ").replace("-", " ")
    words = name_clean.split()

    # 1. Prefix match: code is a prefix of any word in the team name
    #    e.g. "ARK" matches "ARKANSAS", "MICH" matches "MICHIGAN", "NEB" matches "NEBRASKA"
    if any(w.startswith(code_upper) for w in words):
        return True

    # 2. Alias match: code maps to a known substring in the team name
    #    e.g. "SJU" → "john" found in "St. John's Red Storm"
    alias = KALSHI_CODE_ALIASES.get(code_upper)
    if alias and alias.upper() in full_name.upper():
        return True

    # 3. Substring in compressed name (no spaces/punctuation)
    #    e.g. catches edge cases where code spans word boundaries
...
def _match_teams_code_to_event(teams_code, home_full, away_full):
    """Try ALL possible split points of the raw Kalshi teams_code string to find
    a split where left matches one team and right matches the other.

    This handles variable-length NCAAB codes (3 or 4 chars):
      IOWANEB → IOWA+NEB, SJUDUKE → SJU+DUKE, ARKARIZ → ARK+ARIZ, etc.
    """
    tc = teams_code.upper()
    # Try every split from position 2 to len-2 (both halves need ≥2 chars)
    for k in range(2, len(tc) - 1):
        left, right = tc[:k], tc[k:]
        # Try both orderings: left=home,right=away OR left=away,right=home
...
def fetch_ncaab_schedule():
    ...
    try:
        ...
        for event in data:
            ...
            # Parse commence_time → unix timestamp + date key
            ct = commence
            if ct.endswith("Z"):
                ct = ct[:-1] + "+00:00"
...
def get_game_start_ts(team_1, team_2, event_date_str):
    ...
    Returns (unix_timestamp, 'odds_api') or (None, 'ticker_estimate').

    Handles two date offset sources:
    1. ET/UTC mismatch: Kalshi dates by ET, Odds API by UTC. Late games shift +1 day.
    2. Kalshi off-by-one: some NIT/CBI tickers are dated one day before the actual game.
    We check Kalshi date, +1, and +2 to cover both offsets compounding.
    """
    ...
    # Convert event_date (YYYY-MM-DD) to cache date format (YYYYMMDD)
    try:
        dt = datetime.strptime(str(event_date_str), "%Y-%m-%d")
    except (ValueError, TypeError):
        return None, "ticker_estimate"

    # Reconstruct raw teams_code from parsed parts (handles variable-length codes)
    teams_code = (team_1 + team_2).upper()

    # Check the Kalshi date AND next two days (handles two offset sources):
    # 1. ET→UTC date shift: late games (8pm+ ET) cross midnight UTC → stored on next day
    # 2. Kalshi off-by-one: some NIT/CBI markets are dated one day before the game
    # Checking +2 covers both offsets compounding (Kalshi -1 day + UTC +1 day)
    dates_to_check = [
        dt.strftime("%Y%m%d"),                          # Kalshi's ET date
        (dt + timedelta(days=1)).strftime("%Y%m%d"),     # +1: UTC offset for late ET games
        (dt + timedelta(days=2)).strftime("%Y%m%d"),     # +2: Kalshi off-by-one + UTC offset
    ]

    for date_key in dates_to_check:
        # Strategy 1: Direct code-based lookup
        ...
        # Strategy 2: Split-point matching against all events on that date
        ...
        for evt in events_on_date:
            if _match_teams_code_to_event(teams_code, evt["home"], evt["away"]):
                # Cache the match for future lookups within this run
...
def estimate_game_start_from_ticker(event_date_str):
    ...
    Ticker date is in ET. NCAAB games tip off roughly noon–10pm ET (17:00–03:00 UTC).
    We estimate ~7pm ET = midnight UTC next day as the midpoint.
    This gives a 5-hour buffer on either side of actual tip-off.
    ...
    try:
        dt = datetime.strptime(str(event_date_str), "%Y-%m-%d")
        # Game on date X tips off ~7pm ET = ~00:00 UTC on date X+1
...
def get_market_details(market_ticker, team_1=None, team_2=None, event_date=None):
    ...
    Expiration logic:
      - odds_api: orders expire at game start
      - ticker_estimate: estimated ~7pm ET from ticker date, expire 5h before estimate
    """
    try:
        resp = exchange_client.get_market(ticker=market_ticker)
        md = resp.get('market', {})

        # [v4.5] OI: try old field first, fall back to _fp variant (Kalshi renamed these)
        ...
        # [v4.5] Volume: same migration as OI
        vol = md.get('volume')
        ...
        # [v4.5] Game start time from Odds API
        game_start_ts, time_source = get_game_start_ts(team_1, team_2, event_date)

        # [v4.5b] Fallback: estimate from ticker date if Odds API missed
        # Replaces broken close_time fallback (close_time is 14 days out for early-close markets)
        ...
        # Expiration logic — Odds API game time takes priority
        expiration_ts = None
        if game_start_ts:
            if time_source == "odds_api":
                # Exact time: expire at tip-off
                expiration_ts = game_start_ts - ODDS_API_EXPIRATION_BUFFER_SEC
            else:
                # Estimate (~7pm ET ± 5h): expire 5 hours before to be safe
...
def parse_orderbook(orderbook_data):
    ...
    - New fp: [["0.4200", "13.00"], ["0.4000", "5.00"]]  (dollar strings, contract strings)
...
def build_order_objects_for_market(market_row, existing_positions, event_net_exposure,
    ...
    # Market details — [v4.5] now returns game start time from Odds API
    ...
    if expiration_ts is None:
        ...
        return []
    # [v4.5] hours_until_event based on actual game time, not order expiration
    if game_start_ts:
        hours_until_event = (game_start_ts - time.time()) / 3600.0
    else:
        hours_until_event = (expiration_ts - time.time()) / 3600.0
    # Log time source for debugging
    if game_start_ts:
    ...
    print(f"    Time: {hours_until_event:.1f}h ({time_mult:.2f}x) [{time_source}] | OI: {open_interest} ({volume_mult:.2f}x) | Base: {base_mult:.2f}x")

    # Orderbook — [v4.5] handles both legacy and new fp format (March 12 2026 migration)
    ...
    try:
        ...
        # New format: ob_resp['orderbook_fp']['yes_dollars'] = [["0.42", "13.00"], ...]
        # Old format: ob_resp['orderbook']['yes'] = [[42, 13], ...]
        ...
        if not yes_data and not no_data:
            ob_keys = list(ob_resp.keys()) if isinstance(ob_resp, dict) else type(ob_resp).__name__
            print(f"    ⚠️ Orderbook empty — response keys: {ob_keys}")
    except Exception as e:
        print(f"    ⚠️ Orderbook fetch error: {e}")

    # Print orderbook
    yes_ob_str = f"{orderbook_yes_bid}c" if orderbook_yes_bid is not None else "N/A"
    no_ob_str = f"{orderbook_no_bid}c" if orderbook_no_bid is not None else "N/A"
    print(f"    Orderbook: Yes={yes_ob_str}, No={no_ob_str}")
    ...
    # Strategy + Fair value print (matches NBA format)
    myr_str = f"{market_yes_rate*100:.2f}%" if market_yes_rate is not None and not pd.isna(market_yes_rate) else "N/A"
    msz_str = f"n={float(market_sample_size):.1f}" if market_sample_size else "n=0"
    print(f"    Strategy [{PRICING_STRATEGY.title()}]: Market={myr_str} ({msz_str})")
    t1_str = f"{team_1 or '?'}={team_1_yes_rate*100:.2f}%(n={float(team_1_sample_size):.1f})" if team_1_yes_rate is not None and not pd.isna(team_1_yes_rate) else f"{team_1 or '?'}=N/A"
    t2_str = f"{team_2 or '?'}={team_2_yes_rate*100:.2f}%(n={float(team_2_sample_size):.1f})" if team_2_yes_rate is not None and not pd.isna(team_2_yes_rate) else f"{team_2 or '?'}=N/A"
    print(f"                         Teams: {t1_str}, {t2_str}")
    ...
    if yes_total_mult > 0:
        for i, offset in enumerate(yes_offsets):
            ...
            yes_placed += max_here
            ev_type = "H" if is_pairing else "S"
            yes_order_details.append(f"'{bid}c({max_here}c,ev={effective_ev:.0%}{ev_type})'")
            orders.append({
    ...
    if no_total_mult > 0:
        for i, offset in enumerate(no_offsets):
            ...
            no_placed += max_here
            ev_type = "H" if is_pairing else "S"
            no_order_details.append(f"'{bid}c({max_here}c,ev={effective_ev:.0%}{ev_type})'")
            orders.append({
    ...
    if yes_total_mult == 0:
        ...
        shown = yes_order_details[:5]
        extra = f", '... +{len(yes_order_details)-5} more'" if len(yes_order_details) > 5 else ""
        print(f"    → YES: [{', '.join(shown)}{extra}] (mult: {yes_total_mult:.2f}x, placed: {yes_placed})")
    else:
    ...
    if no_total_mult == 0:
        ...
        shown = no_order_details[:5]
        extra = f", '... +{len(no_order_details)-5} more'" if len(no_order_details) > 5 else ""
        print(f"    → NO:  [{', '.join(shown)}{extra}] (mult: {no_total_mult:.2f}x, placed: {no_placed})")
    else:
