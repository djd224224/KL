...
- **3 levels × 5 contracts**, first level **5c off fair**, levels **exactly 2c apart**
...
- **Join, don't lead**: a quote may at most *match* the best **external** level (book net of our
  ...
  the top of the book.
- **Caps** (all enforced): ≤5 contracts per price level per market (hard invariant, `level_cap`
  alert if ever hit); ≤15 contracts (3×5) resting
  per (market, side) — unconditional backstop in `place_with_side_cap()`; **net ±80 per market
  per direction** (`CMM_MAX_POSITION`, position + full ladder ≤ cap); **±500 net per event**
